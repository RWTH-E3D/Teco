...
if __name__ == '__main__':
    ...
    #constants
    path_weather = Path.cwd() / "input" / "weatherdata" / "district1" / "TRY2015_510139065530_Jahr.mos"
    path_citygml = Path.cwd() / "input" / "citygml" / "district1" / "district1_LoD2.gml"

    path_models = Path.cwd() / "output" / "models" / "district1"
    path_results_models = Path.cwd() / "output" / "results_models" / "district1"
    path_results_lca = Path.cwd() / "output" / "results_lca" / "district1"

    prj = Project(load_data=True)
    prj.name = f"district1"
