...
if __name__ == '__main__':
    ...
    #constants
    path_weather = Path.cwd() / "input" / "weatherdata" / "district3" / "TRY2015_535578100702_Jahr.mos"
    path_citygml = Path.cwd() / "input" / "citygml" / "district3" / "district3_LoD2.gml"

    path_models = Path.cwd() / "output" / "models" / "district3"
    path_results_models = Path.cwd() / "output" / "results_models" / "district3"
    path_results_lca = Path.cwd() / "output" / "results_lca" / "district3"

    prj = Project(load_data=True)
    prj.name = f"district3"
