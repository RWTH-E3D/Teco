"""
This script can be used to run over the entire pre_sim_jobs.xlsx input file.
It successively simulates the 81,990 buildings in batches,
with one weather station per project.
Project names are consecutive numbers ("project_X"), to which the input rows of the xlsx file have
been allocated.
This is a parametrized run that exploits the geometrical mean and standard error values of
the given data for non-residential buildings, and assumed minima and maxima for residential buildings.
The following buildings are simulated:

building_type / archetypes / nla / rotation / weather / storey_height / no_storeys / statistic / AHU   / total
    ab             41    *    3  *   5      *   2     *       3                                        =  3,690
    mfh            444   *    3  *   5      *   1     *       3                                        =  19,980
    sfh            468   ...                                                                           =  21,060
 culture            2    * {2;3} *   5      *   6     *       3       *    {2;3}  *      3             =  4,860
 industrial         1    ...                                                                           =  2,430
 retail             5    ...                                                                           =  7,290
 hospitality        9    ...                                                                           =  10,530
 office             4    ...                                                                           =  6,480
 education          2    ...                                                                           =  3,240
 health             1    ...                                                                           =  2,430
                                                                                                       --------
...
- Uses project_name exactly as given (e.g., 'project_8'); each project already has exactly one weather station.
- Filters projects by numeric range: --start-project N --end-project M  (matches project_X where X in [N..M]).
...
EXCEL_PATH = REPO_ROOT / "teco" / "diss_run" / "input" / "pre_sim_jobs_adj.xlsx"
EXCEL_SHEET = "jobs"
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Resumable TEASER→Dymola→TECO per-building runner.")
    ...
    if missing:
        raise KeyError(f"pre_sim_jobs.xlsx is missing required columns: {missing}")
